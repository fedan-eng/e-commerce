  ...
  async function fetchProductsByCategory(category) {
    try {
      ...
      const res = await fetch(
        `/api/products?categories=${encodeURIComponent(category)}&limit=20`,
      );
      const data = await res.json();

      // Randomize the products and take the first 4
      const randomized = shuffleArray(data.products || []).slice(0, 4);
