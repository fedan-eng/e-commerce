...
function extractImageFromContent(content = '') {
  ...
  // Try multiple Substack CDN patterns
  const substackPatterns = [
    /https:\/\/substackcdn\.com\/image\/fetch\/[^"'\s]+/,
    /https:\/\/substack-post-media\.s3\.amazonaws\.com\/[^"'\s]+/,
    /https:\/\/substack\.com\/[^"'\s]+\.(jpg|jpeg|png|webp)/i,
    /https:\/\/[a-z0-9-]+\.substack\.com\/[^"'\s]+\.(jpg|jpeg|png|webp)/i,
  ]
...
function formatSubstackPost(item, index) {
  ...
  // RSS parser may return enclosure as object with attributes or string
  const enclosureUrl = item.enclosure?.url || (typeof item.enclosure === 'string' ? item.enclosure : null)
  ...
  console.log(`[Blog] Post "${item.title}":`, {
    enclosureUrl,
    contentImage,
    finalImage: image,
  })
