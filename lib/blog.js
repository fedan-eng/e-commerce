...
function formatSubstackPost(item, index) {
  ...
  // Build a clean excerpt: prefer description, else strip HTML from content
  ...
  return {
    ...
    readTime,
    views: 0, // Substack RSS doesn't expose view counts
    featured: index === 0,
    substackUrl: item.link, // Always link back to the actual Substack post
  }
}

export async function getBlogPosts() {
  // Use local demo data if no Substack URL is configured
  if (!SUBSTACK_URL) {
  ...
  try {
    const parser = new Parser({
      customFields: {
        item: [
          ['content:encoded', 'contentEncoded'], // properly alias namespaced field
          ['dc:creator', 'creator'],
        ],
      },
    })
